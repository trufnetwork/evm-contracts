...
export const registerAdminTasks = (scope: ConfigurableScopeDefinition) => {
  scope
    .task("proposeAdmin", "Propose a new default admin for the TNOracleV1")
  ...
  scope
    .task("acceptAdmin", "Accept the admin role if the delay period has passed")
    .addParam("contract", "The TNOracleV1 contract address")
  ...
  scope
    .task("renounceAdmin", "Renounces the admin role from the caller's address")
    .addParam("contract", "The TNOracleV1 contract address") 
    .setAction(async ({ contract }, hre) => {
      const tnOracle = await hre.ethers.getContractAt("TNOracleV1", contract) as TNOracleV1;
  ...
  scope
    .task("pause", "Pauses the TNOracleV1 contract")
    .addParam("contract", "The TNOracleV1 contract address")
    .setAction(async ({ contract }, hre) => {
      const tnOracle = await hre.ethers.getContractAt("TNOracleV1", contract) as TNOracleV1;
  ...
  scope
    .task("unpause", "Unpauses the TNOracleV1 contract")
    .addParam("contract", "The TNOracleV1 contract address")
    .setAction(async ({ contract }, hre) => {
      const tnOracle = await hre.ethers.getContractAt("TNOracleV1", contract) as TNOracleV1;
