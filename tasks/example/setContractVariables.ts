...
export const registerSetContractVariablesTasks = (
  scope: ConfigurableScopeDefinition
) => {
  scope
    .task(
      "set-contract-variables",
      "Sets the current variables for the TNConsumerExample contract"
    )
    .addParam("contractAddress", "The deployed contract address")
    .addParam(
      "source",
      "The source key to use",
      SourceKeys.simpleExample,
      sourceEnum,
      true
    )
    .setAction(async (taskArgs, hre) => {
      ...
      const networkSubscriptions =
        SUBSCRIPTION_ID[hre.network.name as keyof typeof SUBSCRIPTION_ID];

      if (!(ownerAddress in networkSubscriptions)) {
        throw new Error(
          `No subscription ID found for ${ownerAddress} on ${hre.network.name}`
        );
      }
      const subscriptionIdValue =
        networkSubscriptions[ownerAddress as keyof typeof networkSubscriptions];
...
/**
 * Returns the encrypted secrets URL from the environment variables.
 * 
 * @returns The encrypted secrets URL
 */
const getEncryptedSecretsUrl = () => {
  dotenv.config();
